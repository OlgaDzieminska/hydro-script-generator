...
from main import ORDERED_HYDRO_MONTHS, PROVIDED_INVALID_WATER_PARAMETER_NAME_ERROR_MESSAGE
...
def addTableContent(table, table_content):
    for state_range, table_data_row in table_content.iterrows():
        ...
        for current_month in ORDERED_HYDRO_MONTHS:
            addStyledContentToCell(row_cells[cell_column_index], table_data_row[str(current_month)])
            cell_column_index += 1

        addStyledContentToCell(row_cells[15], table_data_row['frequency'])
        row_cells[15].merge(row_cells[16])
        row_cells[15].merge(row_cells[17])

        addStyledContentToCell(row_cells[18], table_data_row['lower'])
        row_cells[18].merge(row_cells[19])

        addStyledContentToCell(row_cells[20], table_data_row['higher'])
        row_cells[20].merge(row_cells[21])
...
def appendCzestoscWystapieniaISumyCzasowTrwaniaTableToDocument(document, table_content, parameter_name):
    ...
    addTableContent(czestoscWystapieniaISumyCzasowTrwaniaTable, table_content)
